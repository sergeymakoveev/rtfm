{
	type TreeItem<T extends object> = T & {
		children: TreeItem<T>[];
	};

	/**
	 * Функция сворачивает дерево `root`, начиная со состояния `initialState`,
	 * и применяя `reducer` к каждому элементу в порядке `traversal`
	 */
	function reduceTree<T extends object, S>(
		root: TreeItem<T>,
		reducer: (state: S, item: T) => S,
		initialState: S,
		traversal: 'bfs' | 'dfs-pre-order' | 'dfs-post-order',
	): S {
		let state = initialState;
		const trees = [root];
		while (trees.length) {
			switch (traversal) {
				case 'bfs': {
					const tree = trees.shift();
					if (tree === undefined) {
						break;
					}
					state = reducer(state, tree);
					trees.push(...tree.children);
					break;
				}
				case 'dfs-pre-order': {
					const tree = trees.pop();
					if (tree !== undefined) {
						state = reducer(state, tree);
						trees.push(...tree.children.toReversed());
					}
					break;
				}
				case 'dfs-post-order': {
					let tree = trees.at(-1);
					while (tree && tree.children.length) {
						trees.push(...tree.children.toReversed());
						tree.children = [];
						tree = trees.at(-1);
					}
					if (tree) {
						state = reducer(state, tree);
						trees.pop();
					}
					break;
				}
				default: {
					trees.pop();
				}
			}
		}
		return state;
	}

	type TreeValue = { value: number };
	type State = number[];

	/*
	      1
	  21     22
	31  32 33  34
	*/
	const root: TreeItem<TreeValue> = {
		value: 1,
		children: [
			{
				value: 21,
				children: [
					{
						value: 31,
						children: [],
					},
					{
						value: 32,
						children: [],
					},
				],
			},
			{
				value: 22,
				children: [
					{
						value: 33,
						children: [],
					},
					{
						value: 34,
						children: [],
					},
				],
			},
		],
	};

	const reducer = (acc: State, item: TreeValue): State => [...acc, item.value];

	console.log('bfs:', '\n', [1, 21, 22, 31, 32, 33, 34], reduceTree<TreeValue, State>(root, reducer, [], 'bfs'));
	console.log(
		'dfs-pre-order:',
		'\n',
		[1, 21, 31, 32, 22, 33, 34],
		reduceTree<TreeValue, State>(root, reducer, [], 'dfs-pre-order'),
	);
	console.log(
		'dfs-post-order:',
		'\n',
		[31, 32, 21, 33, 34, 22, 1],
		reduceTree<TreeValue, State>(root, reducer, [], 'dfs-post-order'),
	);

	/**
	 * Возвращает `true`, если переданное значение изменилось по сравнению
	 * с предыдущим рендерингом. При первом рендеринге возвращает `false`.
	 */

	function useHasChanged<T>(value: T): boolean {
		const refPrev = useRef<T>(value);
		const res = refPrev.current !== value;
		refPrev.current = value;
		return res;
	}

	// get /users?sortBy=S&page=P&filterBy=F&pageSize=PS

	type User = {
		name: string;
		status: 'active' | 'blocked' | 'fired';
		divisions: number[];
		groups: number[];
		roles: number[];
	};

	const getList = (
		sortOrder: 'asc' | 'desc',
		sortBy: 'name' | 'group' | 'division',
		pageNumber: number,
		pageSize: 10 | 20 | 50 | 100 | 500,
		filterBy: Record<'status' | 'division' | 'role' | 'group', number[]>,
	): User[] => {};
}
