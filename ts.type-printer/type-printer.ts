	...
	return aliasNames
		.map(aliasName => {
			...
			if (isSkipped) {
				...
				stringifiedType = typeChecker.typeToString(type, undefined, ts.TypeFormatFlags.NoTruncation);

				const _stringifiedType = type.types
					.map(
						/*
                    ...
                    */
						typeInUnion => {
							const propTypeName = typeInUnion.isLiteral()
								? typeChecker.typeToString(typeInUnion, undefined, ts.TypeFormatFlags.NoTruncation)
								: getTypeSignature(typeInUnion);
